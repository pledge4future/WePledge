...
import { FormikHelpers, Formik, useFormik } from "formik";
...
export function ElectricityForm(props: {
  ...
  const electricityDataFormValidationSchema = Yup.object({
    timestamp: Yup.string().required("Please select a Month and Year"),
    groupShare: Yup.number().required("Group Share is required"),
    consumption: Yup.number().required("Consumption is required"),
    energySource: Yup.string().required("Energy Source is required"),
  });

  function onSubmit(values: ElectricityFormValues) {
    const queryParams = {
